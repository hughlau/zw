    ...
    $.page.ajax($.page.getAjaxSettings({
        ...
        , success: function (resultData) {
          //  console.log(resultData)
            ...
            //     $("#airDuctSituation").empty();
            //     for (var i = 0; i < entity.length; i++) {
            //        for(var k in entity[i].operationMaintenanceFormData){
            //         if($(k)){

            //             $(k).value=entity[i].operationMaintenanceFormData[k]
            //         }
            //        }
                    

            //         $("#airDuctSituation").append("<option value='" + entity[i].partCode + "'>" + entity[i].partName + "</option>"); 
            //     }
            // }
        }
    }));
        ...
        $.page.ajax($.page.getAjaxSettings({
            ...
            , success: function (resultData) {
                ...
                if (resultData.status == fw.fwData.FWResultStatus.Success && fw.fwObject.FWObjectHelper.hasValue(resultData.data)) {
                    ...
                    //将数据绑定在页面   modify by lxg 20180321
                    for (var property in entity) {
                        if (fw.fwObject.FWObjectHelper.hasValue(entity[property])) {
                            var id = "#" + property;
                            $(id).val(entity[property]);
                        }
                    }
                    ...
                    //};
                    //if (fw.fwObject.FWObjectHelper.hasValue(entity.taskStatusName)) {
                    //    $("#taskStatusName").val(entity.taskStatusName);
                    //};
