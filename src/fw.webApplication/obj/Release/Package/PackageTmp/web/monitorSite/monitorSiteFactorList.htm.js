...
$.page.pageLoad = function () {
    ...
    if (fw.fwObject.FWObjectHelper.hasValue($.page.params.monitorSiteCode)) {
        ...
        $.page.showTips({ content: "参数monitorSiteCode不能为空！ ", state: "danger" });
    };
    roleViewInit();
};
...
function datagrid1_Load(pageIndex, pageSize, cSettings) {
    ...
    $.page.ajax($.page.getAjaxSettings({
        ...
        , methodName: "queryMonitorSiteFactorList"
        , data: {
            ticket: $.page.ticket
...
function getEquipmentCode() {
    ...
    if (entity && fw.fwObject.FWObjectHelper.hasValue(entity)) {
        monitorFactorCode = entity.monitorFactorCode;
    };
    return monitorFactorCode;
};
...
function getEquipmentCodeList() {
    ...
    if (fw.fwObject.FWObjectHelper.hasValue(entityList) && entityList.length > 0) {
        ...
        for (var i = 0; i < entityList.length; i++) {
            mCodeList.push(entityList[i].monitorFactorCode);
        };
    };
    return mCodeList;
};
...
//打开信息窗口
function openInfo(action, monitorFactorCode) {
    //是否满足条件打开窗口
    var isOpen = true;
    //打开窗口传入的数据
    var data = {
    ...
    //判断传入了记录编码
    if (fw.fwObject.FWObjectHelper.hasValue(monitorFactorCode)) {
        data.monitorFactorCode = monitorFactorCode;
    } else {
        //判断是修改或者是查看信息
        if (action == "edit" || action == "query") {
            //获取选中项编码
            monitorFactorCode = getEquipmentCode();
            //判断选中了项
            if (fw.fwObject.FWObjectHelper.hasValue(monitorFactorCode)) {
                data.monitorFactorCode = monitorFactorCode;
            } else {
    ...
    if (isOpen) {
        //获得传入的参数字符串
        var url = fw.fwUrl.FWUrlHelper.addParams(fw.fwUrl.FWUrlHelper.getAbsoluteUrl("web/monitorSite/monitorSiteFactor.htm", $.page.webSiteRootUrl), data);
        //打开窗口
        mini.open({
            url: url
            , title: "点位因子详情"
        ...
        function (action) {
            if (action == "ok") {
                ...
                $.page.ajax($.page.getAjaxSettings({
                    serviceType: "crossDomainCall"
                    , serviceName: "basicInfo"
                    , methodName: "deleteMBasSiteFactorByFactorCodeList"
                    , data: {
                        ticket: $.page.ticket
                        , monitorFactorCodeList: [entity.monitorFactorCode]
                        , monitorSiteCode: monitorSiteCode
                    }
                    , success: function (resultData) {
                        //判断启用停用成功
