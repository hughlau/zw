...
$.page.pageLoad = function () {
    ...
    buttonEditJQ.on("click", function () {
        ...
        openInfo($.pageCustomer.enumOperate.edit, entity.operationCleanRecordCode);
    });
...
function dataPrint() {
    ...
    var url = "http://101.201.82.103:8083/ticerm/jjfineReportTest.htm?data=" + operatioRecordCodeList.join(',')+"&flag=cleanRecord";
    //打开窗口
    mini.open({
        url: url
        , title: "清掏记录打印"
...
function openInfo(action, dailyMaintenanceTask) {
    ...
    if ($.pageCustomer.hasValue(dailyMaintenanceTask)) {
        data.operationCleanRecordCode = dailyMaintenanceTask;
    };
    var pageParams = { url: "web/OPTask/cleanTaskAdd.htm", width: 800, height: 600, title: "任务信息" };
        ...
        function (action) {
            if (action == "ok") {
                $.page.ajax($.page.getAjaxSettings({
                    serviceType: "crossDomainCall"
                    , serviceName: "operationMaintenance"
                    , methodName: "deleteMaintenanceCleanRecord"
                    , data: {
                        ticket: $.page.ticket
                        , mEntity: entity
                    }
