...
function treegrid1_Renderer(e) {
    ...
    switch (e.field) {
        case "mMenuName":
            html = "<a href=\"javascript:menuExpand()\" style=\"color:blue;\">" + e.value + "</a>";
            break;
        case "mIsHtmlPage":
...
function openInfo(action, mPMenuCode, mMenuCode) {
    ...
    if (isOpen) {
        ...
        //打开窗口
        mini.open({
            url: "/web/sysManage/fwMenu.htm?" + params
            , title: "菜单信息"
            , width: 870
            , height: 612
            , onload: function () {
...
function moveNode(action, mMenuCode) {
    if (action == "up" || action == "down") {
        ...
        if (isOpen) {
            ...
            //打开选择用户窗口
            mini.open({
                url: "/web/sysManage/fwMenuList.htm?" + params
                , title: "选择列表"
                , width: 650
                , height: 380
                , onload: function () {
...
//扩展菜单
function menuExpand() {
    var mMenuCode = getMMenuCode();  //打开窗口
    var data = { ticket: $.page.ticket, mMenuCode: mMenuCode };
    var url = fw.fwUrl.FWUrlHelper.addParams(fw.fwUrl.FWUrlHelper.getAbsoluteUrl("web/sysBasicManage/menuExpand.htm", $.page.webSiteRootUrl), data);
    mini.open({
        url: url
            , title: "菜单信息"
            , width: 870
...
//删除扩展菜单
function delMenuExpand(mMenuCodeList) {
    //获得选中项编码集合
    if (!fw.fwObject.FWObjectHelper.hasValue(mMenuCodeList)) {
        mMenuCodeList = getMMenuCodeList();
    };
            ...
            function (action) {
                if (action == "ok") {
                    ...
                    $.page.ajax($.page.getAjaxSettings({
                        ...
                        , methodName: "deleteMMenuExByMenuCode"
                        , data: {
                            ticket: $.page.ticket
                            , mMenuCodeList: mMenuCodeList
                        }
                        , beforeSend: function () {
                            fw.fwButton.fwButtonHelper.addWait($.page.idM.delMenuExpand);
                        }
                        , success: function (resultData) {
                            //判断删除成功
                            if (resultData.status == fw.fwData.FWResultStatus.Success && resultData.data != null) {
                        ...
                        , complete: function () {
                            fw.fwButton.fwButtonHelper.removeWait($.page.idM.delMenuExpand);
                        }
                    }));
                };
            }
