...
//启用停用选中项
function openSendMessage(mUserIDList) {
    //判断传入了记录编码
    if (!fw.fwObject.FWObjectHelper.hasValue(mUserIDList)) {
        mUserIDList = getMUserIDList();
    };
    ...
    if (mUserIDList) {
        ...
        //打开窗口
        mini.open({
            url: fw.fwUrl.FWUrlHelper.addParams(fw.fwUrl.FWUrlHelper.getAbsoluteUrl("web/sysBasicManage/sendMessage.htm", $.page.webSiteRootUrl), data)
            , title: "发送消息"
            , width: 640
            , height: 512
            , onload: function () {
...
function openInfo(action, mUserID) {
    ...
    if (isOpen) {
        ...
        //打开窗口
        mini.open({
            url: "../sysBasicManage/fwUserInfo.htm?" + params
