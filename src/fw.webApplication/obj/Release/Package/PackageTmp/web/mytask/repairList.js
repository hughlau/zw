...
function onSearch(cSettings) {
    if(buttonClickItem == 0) {
        mini.get("datagrid1").hideColumn("omTime");
        mini.get("datagrid1").hideColumn("maintainers");
    } else {
        mini.get("datagrid1").showColumn("omTime");
        mini.get("datagrid1").showColumn("maintainers");
    }
    //搜集查询条件
    conditionData = $.page.idM.conditionForm.getData();
    conditionData.oprType = buttonClickItem;
...
function dataPrint() {
    ...
    var url = "http://101.201.82.103:8083/ticerm/jjfineReportTest.htm?data=" + operatioRecordCodeList.join(',') + "&flag=maintenanceRecord";
    //打开窗口
    mini.open({
        url: url
        , title: "维修记录打印"
