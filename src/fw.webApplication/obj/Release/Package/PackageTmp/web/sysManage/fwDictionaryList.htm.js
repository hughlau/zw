...
function generateLevelInfo() {
    var mDictionaryTypeCode = $.page.idM.mDictionaryTypeCode.getValue();
    //判断传入编码
    if (fw.fwObject.FWObjectHelper.hasValue(mDictionaryTypeCode)) {
        //启用停用
        $.page.ajax($.page.getAjaxSettings({
            serviceType: "crossDomainCall"
            , serviceName: "sysManage"
            , methodName: "generateLevelInfo"
            , data: {
                ticket: $.page.ticket
                , mDictionaryTypeCode: mDictionaryTypeCode
            }
            , beforeSend: function () {
                fw.fwButton.fwButtonHelper.addWait($.page.idM.generateLevelInfo);
            }
            , success: function (resultData) {
                //判断启用停用成功
                if (resultData.status == fw.fwData.FWResultStatus.Success && resultData.data != null) {
            ...
            , complete: function () {
                fw.fwButton.fwButtonHelper.removeWait($.page.idM.generateLevelInfo);
            }
        }));
    };
};
...
function moveNode(action, mDataID) {
    if (action == "up" || action == "down") {
        ...
        if (isOpen) {
            ...
            //打开选择节点窗口
            mini.open({
                url: "fwDictionaryList.htm?" + params
                , title: "选择列表"
                , width: 650
                , height: 380
                , onload: function () {
