...
function datagrid1_Renderer(e) {
    ...
    switch (e.field) {
        ...
        case "mCallTime":
            ...
            html = "<a href=\"javascript:openInfo('query','" + e.record.mAppLoginLogCode + "')\" style=\"color:blue;\">" + html + "</a>";
            break;
        case "mStatus":
            switch (e.value) {
                case "-9":
...
//打开信息窗口
function openInfo(action, mAppLoginLogCode) {
    //是否满足条件打开窗口
    var isOpen = true;
    //打开窗口传入的数据
    var data = {
    ...
    if (fw.fwObject.FWObjectHelper.hasValue(mAppLoginLogCode)) {
        data.mAppLoginLogCode = mAppLoginLogCode;
    } else {
        //判断是修改或者是查看信息
        if (action == "update" || action == "query") {
            //获取选中项编码
            mAppLoginLogCode = getMAppLoginLogCode();
            //判断选中了项
            if (mAppLoginLogCode && fw.fwObject.FWObjectHelper.hasValue(mAppLoginLogCode)) {
                data.mAppLoginLogCode = mAppLoginLogCode;
            } else {
                isOpen = false;
            };
    ...
    if (isOpen) {
        ...
        var pageUrl = fw.fwUrl.FWUrlHelper.addParams(fw.fwUrl.FWUrlHelper.getAbsoluteUrl("web/sysManage/fwAppLoginLog.htm", $.page.webSiteRootUrl), data); 
        //打开窗口
        mini.open({
            url: pageUrl
            , title: "日志信息"
...
//删除选中项
function del(mAppLoginLogCodeList) {
    //获得选中项编码集合
    if (!fw.fwObject.FWObjectHelper.hasValue(mAppLoginLogCodeList)) {
        mAppLoginLogCodeList = getMAppLoginLogCodeList();
    };
    //判断选中了项
    if (mAppLoginLogCodeList) {
            ...
            function (action) {
                if (action == "ok") {
                    ...
                    $.page.ajax($.page.getAjaxSettings({
                        serviceType: "crossDomainCall"
                        , serviceName: "sysManage"
                        , methodName: "deleteMFWAppLoginLogByMAppLoginLogCodeList"
                        , data: {
                            ticket: $.page.ticket
                            , mAppLoginLogCodeList: mAppLoginLogCodeList
                        }
                        , beforeSend: function () {
