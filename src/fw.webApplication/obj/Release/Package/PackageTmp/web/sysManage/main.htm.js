...
$.page.pageLoad = function () {
    if ($.page.userInfo) {
        $.page.idJQ.aUserInfo.html($.page.userInfo.userName);
    };
    //数加载
    tree1_Load();
};
...
function tree1_Load() {
    ...
    //    , data: {
    //        ticket: $.page.ticket
    //        , mMenuTypeCode: fw.fwData.FWMenuType.SysManage
    //    }
    ...
    $.page.ajax($.page.getAjaxSettings({
        ...
        , data: {
            ticket: $.page.ticket
            , menuTypeCode: fw.m.sysManage.data.FWMenuTypeCode.WebSysManage
        }
        , success: function (resultData) {
            //判断加载数据成功
            if (resultData.status == fw.fwData.FWResultStatus.Success && resultData.data != null) {
