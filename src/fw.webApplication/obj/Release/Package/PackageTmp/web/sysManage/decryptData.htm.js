var jsonlint = function () {
	...
	return typeof a != "undefined" && typeof c != "undefined" && (c.parser = d, c.parse = function () {
		...
		if (typeof onFjson != "undefined")
			var e = a("fs").readFileSync(a("path").join(onFjson.cwd(), d[1]), "utf8");
		else
			var f = a("file").path(a("file").cwd()), e = f.join(d[1]).read({
					charset : "utf-8"
				});
		return c.parser.parse(e)
	}, typeof b != "undefined" && a.main === b && c.main(typeof onFjson != "undefined" ? onFjson.argv.slice(1) : a("system").args)),
	c
}();
...
window.SINGLE_TAB = "  ";
window.ImgCollapsed = "../../resources/scripts/fw/themes/icons/Collapsed.gif";
window.ImgExpanded = "../../resources/scripts/fw/themes/icons/Expanded.gif";
...
function onFjson() {
	...
	var a = $.page.idM.dataInfo.getValue();
	var b = "";
	try {
		if (a == "") {
			a = '""'
		...
		if (c) {
			var d = eval("[" + a + "]");
			b = onFjsonObject(d[0], 0, false, false, false);
...
function onFjsonObject(g, c, d, k, l) {
	var h = "";
	...
	if (IsArray(g)) {
		if (g.length == 0) {
			...
			for (var f = 0; f < g.length; f++) {
				h += onFjsonObject(g[f], c + 1, f < (g.length - 1), true, false)
			};
					...
					if (g.constructor == window._regexpObj.constructor) {
						...
						if (n == 0) {
							...
							for (var b in g) {
								...
								h += GetRow(c + 1, "<span class='PropertyName'>" + a + b + a + "</span>: " + onFjsonObject(g[b], c + 1, ++e < n, false, true))
							}
							o = window.IsCollapsible ? "</span>" : "";
...
function EnsureIsPopulated() {
	if (!$id("Canvas").innerHTML && !!$id("dataInfo").value) {
	    onFjson();
	};
};
...
//解密
function onDecrypt() {
    var dataInfo = $.page.idM.dataInfo.getValue();
    if(fw.fwObject.FWObjectHelper.hasValue(dataInfo)){
        $.page.idM.dataInfo.setValue(fw.fwSafe.FWSafeHelper.decrypt(dataInfo, fw.fwConfig.FWConfigHelper.dataEncryptKey));
...
function onEncrypt() {
    var dataInfo = $.page.idM.dataInfo.getValue();
    if (fw.fwObject.FWObjectHelper.hasValue(dataInfo)) {
        $.page.idM.dataInfo.setValue(fw.fwSafe.FWSafeHelper.encrypt(dataInfo, fw.fwConfig.FWConfigHelper.dataEncryptKey));
    };
};
...
function onIllegal(){
    ...
    if (fw.fwObject.FWObjectHelper.hasValue(dataInfo)) {
        var pattern = /\"(.*)\"/;
        var result = dataInfo.match(pattern);
        if(result[1]){
            $.page.idM.dataInfo.setValue(result[1]);
        };
    };
};
