...
$.page.pageLoad = function () {
    ...
    if (fw.fwObject.FWObjectHelper.hasValue($.page.params.operationMaintenancePersonCode)) {
        personCode = $.page.params.operationMaintenancePersonCode;
    };
    ...
    $.page.idM.datagrid1.on("drawcell", function (e) {
        ...
        if (field == "checkcolumn") {
            if (fw.fwObject.FWObjectHelper.hasValue(record.operationMaintenancePersonCode)) {
...
function onSearch() {
    ...
    conditionData.operationMaintenancePersonCode = personCode; 
...
function datagrid1_Load(pageIndex, pageSize) {
    ...
    $.page.ajax($.page.getAjaxSettings({
        ...
        , methodName: "queryPageMonitorSitePersonAllocator"
        , data: {
            ticket: $.page.ticket
            , pageParams: {
                pageSize: pageSize
...
function inserPlan(codeList) {
    if (fw.fwObject.FWObjectHelper.hasValue(codeList)) {
        $.page.ajax($.page.getAjaxSettings({
            ...
            , methodName: "personMonitorAddAllocator"
            , data: {
                ticket: $.page.ticket,
                operationMaintenancePersonCode: personCode,
...
function del(rowIndex) {
    if (fw.fwObject.FWObjectHelper.hasValue(rowIndex)) {
        var rowEntity = $.page.idM.datagrid1.getRow(rowIndex);
        if (fw.fwObject.FWObjectHelper.hasValue(rowEntity.operationMaintenancePersonCode)) {
            var data = {
                operationMaintenancePersonCode: rowEntity.operationMaintenancePersonCode,
                ...
                function (action) {
                    if (action == "ok") {
                        ...
                        $.page.ajax($.page.getAjaxSettings({
                            serviceType: "crossDomainCall"
                            , serviceName: "operationMaintenance"
                            , methodName: "personMonitorDelAllocator"
                            , data: {
                                ticket: $.page.ticket
                                , entity: data
                            }
...
function delMonitorList(codeList) {
    ...
    if (fw.fwObject.FWObjectHelper.hasValue(codeList)) {
        $.page.ajax($.page.getAjaxSettings({
            serviceType: "crossDomainCall"
            , serviceName: "operationMaintenance"
            , methodName: "personMonitorDelAllocatorList"
            , data: {
                ticket: $.page.ticket,
                operationMaintenancePersonCode: personCode,
