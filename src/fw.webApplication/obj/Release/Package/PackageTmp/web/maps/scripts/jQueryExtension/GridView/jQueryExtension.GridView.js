                    ...
                    if (ValueString != null && ValueString != "" && !isNaN(ValueString) && parseInt(ValueString, 10) != ControlData.PageSize && parseInt(ValueString, 10) > 0) {
                        if (parseInt(ValueString) > $.GridView.MaxPageSize) {
                            jQueryExtension.UI.Alert({
                                ContentHtml: "每页显示条数为1-" + $.GridView.MaxPageSize + "的正整数"
                            });
                            $(this).val($.GridView.MaxPageSize);
                            ValueString = $.GridView.MaxPageSize;
                        };
                        if ($.isFunction(ControlData.Settings.OnPageIndexChanging)) {
                ...
                for (var i = TrRowFrom; i < TrRowTo; i++) {
                    ...
                    for (var j = 0; j < ControlData.SortColumnList.length; j++) {
                        ...
                        if (!fw.fwObject.FWObjectHelper.hasValue(InnerJQ)) {
                            ...
                            } else if (Object.prototype.toString.apply(InnerJQ) == "[object String]") {
                                $("<a>" + InnerJQ + "</a>").appendTo(TdRow_DivRowJQ);
                            } else if (Object.prototype.toString.apply(InnerJQ) == "[object Number]") {
                                $("<a>" + InnerJQ + "</a>").appendTo(TdRow_DivRowJQ);
                            } else if (Object.prototype.toString.apply(InnerJQ) == "[object Object]") {
                                $("<a>&nbsp;</a>").appendTo(TdRow_DivRowJQ);
                            } else {
                                $("<a>&nbsp;</a>").appendTo(TdRow_DivRowJQ);
                            };
