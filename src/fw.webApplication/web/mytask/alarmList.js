...
$.page.pageLoad = function () {
    ...
    //$.page.idM.datagrid1.hideColumn("prescribeRepairTime");
    $.page.idM.datagrid1.hideColumn("operationMaintenancePersonName");
    $.page.idM.datagrid1.hideColumn("operationMaintenanceUnitName");
    $.page.idM.datagrid1.hideColumn("isSolve");
    $.page.idM.datagrid1.hideColumn("repairTime");
    ...
    buttonDeleteJQ.bind("click", function () {
        ...
        for (var item in Entity) {
            if (Entity[item].isSolve == 1) {
                haSolved = true;
                break;
            }
        }
        if (haSolved) {
            mini.alert("包含任务已经解决的记录,不可删除。");
        } else {
            deleteInfos();
        };
    });
            ...
            function (action) {
                if (action == "ok") {
                    $.page.ajax($.page.getAjaxSettings({
                        serviceType: "crossDomainCall"
                        , serviceName: "operationMaintenance"
                        , methodName: "deleteOpMaintenanceTask"
                        , data: {
                            ticket: $.page.ticket
                           , mEntity: entity
                        }
...
function openMaintenanceTemplate() {
    ...
    if (entity.operationMaintenanceFormFileName == undefined || null == entity.operationMaintenanceFormFileName || "" == entity.operationMaintenanceFormFileName) {
        pageUrl ="alarmTask.htm";
    }
    var pageParams = { url: "web/mytask/" + pageUrl, width: 800, height: 600, title: "监测点位" };
    $.pageCustomer.openPage(data, pageParams);
};
