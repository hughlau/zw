...
$.page.pageInit = function () {
    ...
    $.page.dataSourceSettingsDictionary = {
        "mDictionaryTypeCode": {
            dataSourceName: "sysManage.getDictionaryMFWDictionaryType"
            , dataSourceParams: { ticket: $.page.ticket, isExtendTable: 1 }
        }
    };
};
    ...
    $.page.ajax($.page.getAjaxSettings({
        ...
        , success: function (resultData) {
            var columns = [
                ...
                { field: "mDataID", headerAlign: "center", align: "left", header: "数据ID", visible: false },
                { field: "mDictionaryTypeCode", headerAlign: "center", align: "left", header: "字典类型编码" },
                { field: "mDictionaryTypeName", headerAlign: "center", align: "left", header: "字典类型名称" },
                { field: "mPCode", headerAlign: "center", align: "left", header: "父级编码" },
                { field: "mCode", headerAlign: "center", align: "left", header: "编码" },
                { field: "mName", name: "mName", headerAlign: "center", align: "left", header: "名称", renderer: "treegrid1_Renderer" }
            ];
            if (resultData.status == fw.fwData.FWResultStatus.Success && resultData.data != null) {
                for (var i = 0; i < resultData.data.length; i++) {
                    var entity = resultData.data[i];
                    columns.push({
                        field: entity.mColumnName
                        , header: entity.mColumnChineseName
                    });
                };
            };
            fw.fwArrayHelper.FWArrayHelper.addRange(columns, [
                { field: "mIx", headerAlign: "center", align: "center", header: "顺序" },
                { field: "mIsDis", headerAlign: "center", align: "center", header: "停用状态", renderer: "treegrid1_Renderer" }
            ]);
...
function treegrid1_Load(pageIndex, pageSize) {
    ...
    $.page.ajax($.page.getAjaxSettings({
        serviceType: "crossDomainCall"
        , serviceName: "sysManage"
        , methodName: "queryTableMFWDictionaryExtendTable"
        , data: {
            ticket: $.page.ticket
            , mDictionaryTypeCode: conditionData.mDictionaryTypeCode
        }
...
function openInfo(action, mPCode, mDataID) {
    ...
    if (isOpen) {
        //获得传入的参数字符串 
        var pageUrl = fw.fwUrl.FWUrlHelper.addParams(fw.fwUrl.FWUrlHelper.getAbsoluteUrl("web/sysManage/fwDictionaryExtendTable.htm", $.page.webSiteRootUrl), data); 
