...
$.page.pageInit = function () {
    ...
    $.page.dataSourceSettingsDictionary = {
        "mStatus": {
            ...
            , dataSourceParams: { pCode: "FWResultAppInstallInfoLogCallStatus" }
        },
        "mAppInstallStatus": {
            dataSourceName: "sysManage.getDictionary"
            , dataSourceParams: { pCode: "FWResultAppInstallInfoLogStatus" }
        }
    };
};
...
function datagrid1_Renderer(e) {
    ...
    switch (e.field) {
        ...
        case "mCallTime":
            ...
            html = "<a href=\"javascript:openInfo('query','" + e.record.mAppInstallInfoLogCode + "')\" style=\"color:blue;\">" + html + "</a>";
            break;
        case "mStatus":
            switch (e.value) {
                case "0":
...
function datagrid1_Load(pageIndex, pageSize) {
    ...
    $.page.ajax($.page.getAjaxSettings({
        ...
        , methodName: "queryPageMFWAppInstallInfoLogList"
        , data: {
            ticket: $.page.ticket
            , pageParams: {
                pageSize: pageSize
...
//获得选中项编码，只能选择一项
function getMAppInstallInfoLogCode() {
    var mAppInstallInfoLogCode = undefined, isSelectOne = false;
    //获取选中项对象集合
    var entityList = $.page.idM.datagrid1.getSelecteds();
    //判断对象有值
    if (fw.fwObject.FWObjectHelper.hasValue(entityList) && entityList.length > 0) {
        if (entityList.length == 1) {
            isSelectOne = true;
            mAppInstallInfoLogCode = entityList[0].mAppInstallInfoLogCode;
        };
    };
    if (!isSelectOne) {
        mini.alert("请选择一项！");
        mAppInstallInfoLogCode = undefined;
    };
    return mAppInstallInfoLogCode;
};

//获得选中项编码集合
function getMAppInstallInfoLogCodeList() {
    var mAppInstallInfoLogCodeList = undefined;
    //获取选中项对象集合
    var entityList = getSelectedEntityList();
    //判断对象集合有值
    if (fw.fwObject.FWObjectHelper.hasValue(entityList) && entityList.length > 0) {
        mAppInstallInfoLogCodeList = [];
        for (var i = 0; i < entityList.length; i++) {
            mAppInstallInfoLogCodeList.push(entityList[i].mAppInstallInfoLogCode);
        };
    };
    return mAppInstallInfoLogCodeList;
};

//打开信息窗口
function openInfo(action, mAppInstallInfoLogCode) {
    //是否满足条件打开窗口
    var isOpen = true;
    //打开窗口传入的数据
    var data = {
    ...
    if (fw.fwObject.FWObjectHelper.hasValue(mAppInstallInfoLogCode)) {
        data.mAppInstallInfoLogCode = mAppInstallInfoLogCode;
    } else {
        //判断是修改或者是查看信息
        if (action == "update" || action == "query") {
            //获取选中项编码
            mAppInstallInfoLogCode = getMAppInstallInfoLogCode();
            //判断选中了项
            if (mAppInstallInfoLogCode && fw.fwObject.FWObjectHelper.hasValue(mAppInstallInfoLogCode)) {
                data.mAppInstallInfoLogCode = mAppInstallInfoLogCode;
            } else {
                isOpen = false;
            };
    ...
    if (isOpen) {
        ...
        //打开窗口
        mini.open({
            url: "fwAppInstallInfoLog.htm?" + params
            , title: "安装日志信息"
            , width: 640
            , height: 400
            , onload: function () {
...
//删除选中项
function del(mAppInstallInfoLogCodeList) {
    //获得选中项编码集合
    if (!fw.fwObject.FWObjectHelper.hasValue(mAppInstallInfoLogCodeList)) {
        mAppInstallInfoLogCodeList = getMAppInstallInfoLogCodeList();
    };
    //判断选中了项
    if (mAppInstallInfoLogCodeList) {
            ...
            function (action) {
                if (action == "ok") {
                    ...
                    $.page.ajax($.page.getAjaxSettings({
                        serviceType: "crossDomainCall"
                        , serviceName: "sysManage"
                        , methodName: "deleteMFWAppInstallInfoLogByMAppInstallInfoLogCodeList"
                        , data: {
                            ticket: $.page.ticket
                            , mAppInstallInfoLogCodeList: mAppInstallInfoLogCodeList
                        }
                        , beforeSend: function () {
