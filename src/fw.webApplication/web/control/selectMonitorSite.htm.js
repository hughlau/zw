...
$.page.pageLoad = function () {
    ...
    $.page.idM.datagrid1.on("select", function (e) {
        var cantonJQ = $("#monitorSelectedList");
        $('<a class="select" style="margin-right:5px" id="' + e.record.monitorSiteCode + '" href="#">' + e.record.monitorSiteName + '</a>').appendTo(cantonJQ);

    })
    $.page.idM.datagrid1.on("deselect", function (e) {
        $("#" + e.record.monitorSiteCode).remove();
    })
};
...
function getSelectedEntity() {
    var selectMonitors = $("#monitorSelectedList").children("a");
    var arrMs = new Array();
    if (selectMonitors.length > 0) {
        selectMonitors.each(function () {
            var item = {
                code: this.id,
                name: this.innerHTML
            }
    ...
    if (!$.pageCustomer.hasValue(arrMs)) {
        mini.alert("请选择一项！");
    };
    return arrMs;
};
