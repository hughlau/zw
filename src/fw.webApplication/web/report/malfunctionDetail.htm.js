...
function onSearch() {
    if ($.pageCustomer.hasValue(malfunctionCount))
        var maxCount = malfunctionCount > 20 ? 20 : malfunctionCount;
    for (var i = 0; i < maxCount; i++) {
        var item = {};
        item.monitorSiteName =  monitorSiteName;
        item.cantonName = cantonName;
        item.malfunctionDate = getRandomDateBetween(fw.fwDateTime.FWDateTimeHelper.addMonths(new Date(), -8), new Date());
        item.malfunctionReason = "设施故障明细";
