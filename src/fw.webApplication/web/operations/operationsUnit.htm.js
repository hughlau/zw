        ...
        $.page.ajax($.page.getAjaxSettings({
            ...
            , success: function (resultData) {
                ...
                if (resultData.status == fw.fwData.FWResultStatus.Success && resultData.data != null) {
                    ...
                    mini.get("unitManagerName").setValue(entity.unitManagerName);
                    mini.get("unitManagerID").setValue(entity.unitManagerID);
                    $("#td_unitManagerName .mini-buttonedit-input").val(entity.unitManagerName);
                    $("#td_unitManagerID .mini-textbox").val(entity.unitManagerID);
...
function onButtonChooseManager(e) {
    var textMonitor = this;
    var data = { ticket: $.page.ticket, selectType: fw.fwData.FWSelectType.Single, selectCallback: "onManagerselectCallback" };
    var pageParams = { url: "web/operations/selectUnitManager.htm", width: 800, height: 600, title: "运维管理员" };
    $.page.openPage(data, pageParams);
};

function onManagerselectCallback(callbackData) {
    if ($.page.hasValue(callbackData)) {
        mini.get("unitManagerID").setValue(callbackData.userID);
        $("#unitManagerID").val(callbackData.userID);
        $.page.idM.btnChooseMonitor = new mini.Form("#unitManagerName")      
        $("#unitManagerName .mini-buttonedit-input").val(callbackData.userName)
        $("#unitManagerName .mini-buttonedit-input").attr('disabled', 'disabled');
        $.page.idM.btnChooseMonitor.validate(); 
    };
};