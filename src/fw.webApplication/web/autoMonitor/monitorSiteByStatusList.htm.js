...
function onSearch(cSettings) {
    //管理员查看报停设备显示移除按钮
    if ($.page.idM.statusCode.value == '13' && fw.fwCookie.FWCookieHelper("login_role") == 'sysAdminRole') {
        $.page.idM.moveBT.show();
        mini.get("datagrid1").showColumn("checkcolumn");
    } else {
        $.page.idM.moveBT.hide();
        mini.get("datagrid1").hideColumn("checkcolumn");
    }
    //搜集查询条件
