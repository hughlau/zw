$.TouchGridView = {
    ...
    , Column: function () {
        return {
            ...
            , Color: ''                                              //颜色 用于显示效果
            , Width: $.TouchGridView.ColumnMinWidth                       //列宽度
    ...
    , Settings: function () {
        return {
            ...
            , Height: null                                          //数据表高度
            , LoadingDelay: 200                                    //加载延时，当加载时间小于该时间时，锁定会延时到该时间消失
    ...
    , FootFunction: function (Properties) {
        ...
        if (fw.fwObject.FWObjectHelper.hasValue(ControlData)) {
            ...
            var PageCount = ControlData.Settings.RecordCount % ControlData.PageSize == 0 ? parseInt(ControlData.Settings.RecordCount / ControlData.PageSize, 10) : parseInt(ControlData.Settings.RecordCount / ControlData.PageSize, 10) + 1;
            if (fw.fwObject.FWObjectHelper.hasValue(ControlData)) {
    ...
    , GetColumnWidth: function (Properties) {
        var Settings = {
            Selector: null                                       //GridView控件选择器
            , HeadText: null
            , MinWidth: $.TouchGridView.ColumnMinWidth
            , MaxWidth: null
            , StaticColumnWidth: null
            , DynamicColumnCount: null
            , EveryWordWidth: 12
        };
        $.extend(Settings, Properties);

        if (fw.fwObject.FWObjectHelper.hasValue(Settings.Selector) && fw.fwObject.FWObjectHelper.hasValue(Settings.StaticColumnWidth) && fw.fwObject.FWObjectHelper.hasValue(Settings.DynamicColumnCount)) {
            Settings.DynamicColumnCount = parseInt(Settings.DynamicColumnCount, 10);
            var GridViewWidth = $(Settings.Selector).width() - 60 - parseInt(Settings.StaticColumnWidth, 10) - Settings.DynamicColumnCount;
            var AverageWidth = GridViewWidth / Settings.DynamicColumnCount;
            Settings.MinWidth = AverageWidth;
        };
        var SplitArray = ["<div", "</div", "<p", "</p", "<br"]
        var TextArray = [];
        var TextTempArray = [Settings.HeadText];
        for (var i = 0; i < SplitArray.length; i++) {
            TextArray = [];
            for (var j = 0; j < TextTempArray.length; j++) {
                var StringArray = TextTempArray[j].split(SplitArray[i]);
                for (var k = 0; k < StringArray.length; k++) {
                    TextArray.push(StringArray[k]);
                };
            };
        ...
        for (var i = 0; i < TextArray.length; i++) {
            ...
            var W = GetWordLength(TextArray[i]) * Settings.EveryWordWidth;
            if (W > Width) {
                Width = W;
            };
        };
        if (fw.fwObject.FWObjectHelper.hasValue(Settings.MaxWidth)) {
            Settings.MaxWidth = parseInt(Settings.MaxWidth, 10);
            if (Settings.MaxWidth < Width) {
                Width = Settings.MaxWidth;
            };
        };
        if (fw.fwObject.FWObjectHelper.hasValue(Settings.MinWidth)) {
            Settings.MinWidth = parseInt(Settings.MinWidth, 10);
            if (Settings.MinWidth > Width) {
    ...
    , GetTrRowSelectedJQ: function (Properties) {
     ...
     , GetSelectedEntity: function (Properties) {
         ...
         var Entity = $.TouchGridView.GetTrRowSelectedJQ({ Selector: Settings.Selector }).data("Entity");
         if (Entity == undefined) {
             Entity = null;
         };
         return Entity;
     ...
     , ClearTrRowSelected: function (Properties) {
         var Settings = {
             Selector: null                                       //GridView控件选择器
         };
         $.extend(Settings, Properties);
         ...
         var OldSelectedIndex = ControlData.ControlJQs.TrDataRowArrowJQ.index(trOldJQ);
     ...
     , GetPageSize: function (Properties) {
         ...
         Settings.PageSize = Settings.PageSize > $.TouchGridView.MaxPageSize ? $.TouchGridView.MaxPageSize : Settings.PageSize;
         var ControlData = $(Settings.Selector).data("ControlData");
         if (fw.fwObject.FWObjectHelper.hasValue(ControlData)) {
     ...
     , ColumnSetBind: function (Properties) {
         var Settings = {
             Selector: null                                       //GridView控件选择器
         };
         $.extend(Settings, Properties);
         var ControlData = $(Settings.Selector).data("ControlData");
         if (fw.fwObject.FWObjectHelper.hasValue(ControlData)) {
             ControlData.ControlJQs.ColumnSetJQ.ContextMenuStrip(
             {
                 TextField: 'HeadText'
                 , ItemsField: "ChildColumnList"
                 , ClickField: "MenuClick"
                 , CheckboxField: "IsShow"
                , Items: function () {
                    var Items = [];
                    for (var i = 0; i < ControlData.SortColumnList.length; i++) {
                        var Column = ControlData.SortColumnList[i];
                        Column.MenuClick = function (e) {
                ...
                , EventArray: [
                    {
                        ...
                        , Function: function (e, thisJQ) {
                            var thisPosition = jQueryExtension.Position(thisJQ);
                            var thishBox = jQueryExtension.Box(thisJQ);
                            var EventReturn = undefined;
                            EventReturn = {
                                Event: e
                                , Left: thisPosition.AbsoluteLeft
                                , Top: thisPosition.AbsoluteTop + thishBox.Height()
                                , Width: thishBox.Width()
                                , Height: thishBox.Height()
                            };
                            return EventReturn;
                        }
                    }
         ...
         if (!fw.fwObject.FWObjectHelper.hasValue(ColumnSetData)) {
             ...
             ColumnSetData.ControlJQs.GridView_DivTableColumnSet_SingleColumnSet_SortType_AscJQ.bind("click", function () {
                 ...
                 if (ColumnEntity.SortType != "Asc") {
                     ...
                     $.TouchGridView.Refresh({ Selector: ControlData.ControlJQs.GridViewJQ });
                     GridView_DivTableColumnSetJQ.hide();
                 };
             });
         ...
         for (var i = 0; i < ControlData.SortColumnList.length; i++) {
             ...
             $("<input type=\"checkbox\" class=\"GridView_TrHead_ThTableColumnSet_divTableColumnSet_IsShow\" " + (ControlData.SortColumnList[i].IsShow ? "checked=\"checked\"" : "") + " />").bind("click", function () {
                 ...
                 if (CheckedCheckboxJQ.length < 1) {
                     ...
                     $.TouchGridView.UpdateSortColumnList({ Selector: ControlData.ControlJQs.GridViewJQ });
                     $.TouchGridView.UpdateLayout({ Selector: ControlData.ControlJQs.GridViewJQ });
                 };
             }).appendTo(Div0JQ);
     ...
     , Select: function (Properties) {
         ...
         if (fw.fwObject.FWObjectHelper.hasValue(ControlData)) {
             ...
             if (Settings.SelectedIndex > -1) {
                 ...
                 SelectedIndex = ControlData.SelectedIndex >= ControlData.Settings.DataSource.length ? ControlData.Settings.DataSource.length - 1 : ControlData.SelectedIndex;
             };
             ControlData.ControlJQs.TrDataRowArrowJQ.eq(SelectedIndex).click();
         };
     }
     ...
     , UpdateLayout: function (Properties) {
         ...
         if (fw.fwObject.FWObjectHelper.hasValue(ControlData)) {
             ...
             var HeadHeight = $.TouchGridView.GetHeadHeight({ Selector: ControlData.ControlJQs.GridViewJQ });
             ...
             if (ControlData.ControlJQs.DivFoot1JQ.width() / 2 < ControlData.ControlJQs.DivFoot1RightJQ.width()) {
                 ControlData.ControlJQs.DivFoot1LeftJQ.hide();
             } else {
                 ControlData.ControlJQs.DivFoot1LeftJQ.show();
             };
         };
     }
...
//分页GridView
$.fn.extend({
    TouchGridView_Init: function (Properties) {
        if (!fw.fwObject.FWObjectHelper.hasValue(Properties)) { Properties = {}; };
        var Settings = $.TouchGridView.Settings();
        $.extend(Settings, Properties);
        this.each(function () {
            var GridViewJQ = $(this);
            ...
            if (!fw.fwObject.FWObjectHelper.hasValue(ControlData)) {
                ...
                $.TouchGridView.DivGridView_ContentRandomNumber = "GridView_TableBody_Scroll__" + WindowGuid;
                $.TouchGridView.DivGridView_IndexRandomNumber = "GridView_TableBody_Lock__" + WindowGuid;
                var Html = "";
                ...
                ControlData.ControlJQs.DivFoot1LeftJQ = $(">div.jQE_UI_GridView_Foot1Left", ControlData.ControlJQs.DivFoot1JQ).hide();
                ControlData.ControlJQs.DivFoot1RightJQ = $(">div.jQE_UI_GridView_Foot1Right", ControlData.ControlJQs.DivFoot1JQ);
                ...
                ControlData.ControlJQs.DivHeadScrollBarJQ.width(Settings.ScrollBarWidth);
                ...
                ControlData.ControlJQs.Foot1_PageSizeJQ.bind("blur", function () {
                    ...
                    if (ValueString != null && ValueString != "" && !isNaN(ValueString) && parseInt(ValueString, 10) != ControlData.PageSize && parseInt(ValueString, 10) > 0) {
                        if (parseInt(ValueString, 10) > $.TouchGridView.MaxPageSize) {
                            jQueryExtension.UI.Alert({
                                ContentHtml: "每页显示条数为1-" + $.TouchGridView.MaxPageSize + "的正整数"
                            });
                            $(this).val($.TouchGridView.MaxPageSize);
                            ValueString = $.TouchGridView.MaxPageSize;
                        };
                        if ($.isFunction(ControlData.Settings.OnPageIndexChanging)) {
                            ControlData.PageSize = parseInt(ValueString, 10);
                            ControlData.Settings.OnPageIndexChanging(ControlData.PageNow);
                        };
                    } else {
                        if (ValueString != null && ValueString != "" && !isNaN(ValueString) && parseInt(ValueString, 10) > 0) {
                        }
                        else {
                            jQueryExtension.UI.Alert({
                                ContentHtml: "每页显示条数为1-" + $.TouchGridView.MaxPageSize + "的正整数"
                ...
                ControlData.ControlJQs.Foot1_PageNowJQ.bind("blur", function () {
                    var ValueString = $(this).val();
                    if (ValueString != null && ValueString != "" && !isNaN(ValueString) && parseInt(ValueString, 10) != ControlData.PageNow && parseInt(ValueString, 10) > 0) {
                        if ($.isFunction(ControlData.Settings.OnPageIndexChanging)) {
                            ControlData.PageNow = parseInt(ValueString, 10);
                            if (ControlData.Settings != null && ControlData.Settings.RecordCount != null) {
                                var PageCount = parseInt(ControlData.Settings.RecordCount / ControlData.PageSize) + 1;
                                if (ControlData.PageNow > PageCount) {
                                    ControlData.PageNow = PageCount;
                                };
                            };
                            ControlData.Settings.OnPageIndexChanging(ControlData.PageNow);
                        };
                ...
                if ($.browser.msie && $.browser.version == "6.0") {
                    ControlData.ControlJQs.GridViewJQ.bind("jQueryExtension_Event_Resize", function () {
                        var ControlData = $(this).data("ControlData");
                        if (fw.fwObject.FWObjectHelper.hasValue(ControlData)) {
                            var Height = ControlData.ControlJQs.GridViewJQ.height();
                            var HeadHeight = ControlData.ControlJQs.DivHeadJQ.is(":hidden") ? 0 : ControlData.ControlJQs.DivHeadJQ.height();
                            var FootHeight = ControlData.ControlJQs.DivFootJQ.is(":hidden") ? 0 : ControlData.ControlJQs.DivFootJQ.height();
                            var BodyHeight = Height - HeadHeight - FootHeight;
                            ControlData.ControlJQs.DivBodyJQ.height(BodyHeight);
                            //                            $("#liInfo").html(Height + "|" + HeadHeight + "|" + BodyHeight + "|" + FootHeight);
                        };
                    });
                };
            } else {
            ...
            if (!Settings.IsShowHead) {
                ...
                ControlData.ControlJQs.DivBodyJQ.css("top", "0px");
            } else {
                ControlData.ControlJQs.DivHeadJQ.show();
                ControlData.ControlJQs.DivBodyJQ.css("top", ControlData.ControlJQs.DivHeadJQ.height() + "px");
            };

            //是否显示分页
            if (!Settings.IsShowFoot) {
                ControlData.ControlJQs.DivFootJQ.hide();
                ControlData.ControlJQs.DivBodyJQ.css("bottom", "0px");
            } else {
                ControlData.ControlJQs.DivFootJQ.show();
                ControlData.ControlJQs.DivBodyJQ.css("bottom", ControlData.ControlJQs.DivFootJQ.height() + "px");
            };
                ...
                if (OldColumnList.length == Settings.ColumnList.length) {
                    for (var i = 0; i < OldColumnList.length; i++) {
                        ...
                        for (var j = 0; j < Settings.ColumnList.length; j++) {
                            if (fw.fwObject.FWObjectHelper.hasValue(OldColumnList[i]) && OldColumnList[i].HeadText == Settings.ColumnList[j].HeadText && OldColumnList[i].DataField == Settings.ColumnList[j].DataField) {
                                IsHas = true;
                                break;
                            };
                        };
            ...
            if (IsColumnListChange) {
                ...
                var ColumnSettingsInit = function (ColumnList) {
                    ...
                    var Init = function (ColumnList, ParentColumn) {
                        for (var i = 0; i < ColumnList.length; i++) {
                            ...
                            var ColumnSettings = $.TouchGridView.Column();
                            $.extend(ColumnSettings, Column);
                            if (!fw.fwObject.FWObjectHelper.hasValue(ColumnSettings.Width)) { ColumnSettings.Width = $.TouchGridView.ColumnMinWidth; };
                            ColumnSettings.Width = parseInt(ColumnSettings.Width, 10);
                ...
                var HeadFunction = function (TbodyHeadJQ, Column) {
                    var Function = function (Column, Index) {
                        ...
                        $("<div class=\"jQE_UI_GridView_Arrow_ColumnSet\" title=\"设置\">&nbsp;</div>").appendTo(thJQ);
                        if (!Column.IsShow) {
                            thJQ.hide();
                        };
                    };
                ...
                $("<th rowspan=\"" + AllMaxLevel + "\"><div class=\"jQE_UI_GridView_Arrow_Select\">&nbsp;</div><div class=\"jQE_UI_GridView_Arrow_ColumnSet Default\" title=\"设置\">&nbsp;</div></th>").appendTo($(">tr:eq(0)", ControlData.ControlJQs.TbodyHeadArrowJQ));
                ...
                var thJQ = $("<th rowspan=\"" + AllMaxLevel + "\" style=\"border-right-width:0px;\"><div style=\"width:" + ControlData.Settings.ScrollBarWidth + "px\">&nbsp;</div></th>").appendTo($(">tr:eq(0)", ControlData.ControlJQs.TbodyHeadScrollJQ));
                ...
                var thJQ = $("<th rowspan=\"" + AllMaxLevel + "\" style=\"border-right-width:0px;\"><div style=\"width:" + ControlData.Settings.ScrollBarWidth + "px\">&nbsp;</div></th>").appendTo($(">tr:eq(0)", ControlData.ControlJQs.TbodyHeadLockRightJQ));

                ControlData.ControlJQs.ColumnSetJQ = $(">tbody>tr>th>div.jQE_UI_GridView_Arrow_ColumnSet", ControlData.ControlJQs.HeadTableJQ);
                $.TouchGridView.ColumnSetBind({ Selector: ControlData.ControlJQs.GridViewJQ });
            ...
            if (ControlData.Settings.IsFillBodyRow) {
                $.TouchGridView.FillBodyRowFunction({ Selector: ControlData.ControlJQs.GridViewJQ, FillRowCount: null });
            };

            //打开加载锁定屏
            ControlData.ControlJQs.GridViewJQ.TouchGridView_Loading();
        });
        return this;
    }
    , TouchGridView_Loading: function () {
        this.each(function () {
            var ControlData = $(this).data("ControlData");
            if (fw.fwObject.FWObjectHelper.hasValue(ControlData)) {
    ...
    , TouchGridView: function (Properties) {
        if (!fw.fwObject.FWObjectHelper.hasValue(Properties)) { Properties = {}; };
        var Settings = $.TouchGridView.Settings();
        $.extend(Settings, Properties);
        ...
        this.each(function () {
            var GridViewJQ = $(this);
            //初始化控件
            GridViewJQ.TouchGridView_Init(Settings);
            ...
            if (fw.fwObject.FWObjectHelper.hasValue(ControlData)) {
                    ...
                    for (var j = 0; j < ColumnList.length; j++) {
                        ...
                        if (Column.ChildColumnList.length > 0) {
                            ...
                            if (!fw.fwObject.FWObjectHelper.hasValue(InnerJQ)) {
                                ...
                                switch (Object.prototype.toString.apply(InnerJQ)) {
                                    case "[object String]":
                ...
                if (ControlData.Settings.IsFillBodyRow && TrRowFillTo > 0) {
                    $.TouchGridView.FillBodyRowFunction({ Selector: ControlData.ControlJQs.GridViewJQ, FillRowCount: TrRowFillTo, RowIndex: RowIndex });
                };
                ...
                //                //IE6.0显示Bug
                //                if ($.browser.msie && $.browser.version == "6.0") {
                //                    ControlData.ControlJQs.DivArrowTouchScrollJQ.hide();
                //                    ControlData.ControlJQs.DivLockLeftTouchScrollJQ.hide();
                //                    ControlData.ControlJQs.DivScrollTouchScrollJQ.hide();
                //                    ControlData.ControlJQs.DivLockRightTouchScrollJQ.hide();
                //                    ControlData.ControlJQs.DivArrowTouchScrollJQ.show();
                //                    ControlData.ControlJQs.DivLockLeftTouchScrollJQ.show();
                //                    ControlData.ControlJQs.DivScrollTouchScrollJQ.show();
                //                    ControlData.ControlJQs.DivArrowTouchScrollJQ.show();
                //                };

                //重新生成分页
                $.TouchGridView.FootFunction({ Selector: ControlData.ControlJQs.GridViewJQ });
                ...
                //整理数据行的高度
                $.TouchGridView.UpdateRowHeight({ Selector: ControlData.ControlJQs.GridViewJQ });
                ...
                if (ControlData.Settings.AllowRowSelected && ControlData.ControlJQs.TrDataRowArrowJQ.length > 0) {
                    ...
                    ControlData.ControlJQs.TrDataRowLockLeftJQ.bind("click", function (e) {
                        ...
                        $.TouchGridView.Select({ Selector: ControlData.ControlJQs.GridViewJQ, SelectedIndex: SelectedIndex });
                    });
